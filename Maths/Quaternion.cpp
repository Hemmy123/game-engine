...
Quaternion Quaternion::EulerAnglesToQuaternion(float pitch, float yaw, float roll)	{
	float y2 = (float)MathUtils::DegToRad(yaw/2.0f);
	float p2 = (float)MathUtils::DegToRad(pitch/2.0f);
	float r2 = (float)MathUtils::DegToRad(roll/2.0f);
...
Quaternion Quaternion::AxisAngleToQuaterion(const Vector3& vector, float degrees)	{
	float theta = (float)MathUtils::DegToRad(degrees);
...
Quaternion Quaternion::FromMatrix(const Matrix4 &m)	{
	...
	q.w = sqrt(MathUtils::max(0.0f, (1.0f + m.values[0] + m.values[5] + m.values[10]))) / 2.0f;
	q.x = sqrt(MathUtils::max(0.0f, (1.0f + m.values[0] - m.values[5] - m.values[10]))) / 2.0f;
	q.y = sqrt(MathUtils::max(0.0f, (1.0f - m.values[0] + m.values[5] - m.values[10]))) / 2.0f;
	q.z = sqrt(MathUtils::max(0.0f, (1.0f - m.values[0] - m.values[5] + m.values[10]))) / 2.0f;
